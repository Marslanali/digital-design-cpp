...
void test_arithmetic(const std::string file_path, const int input_a_size, const int input_b_size, const int output_size) {
  ...
  if (input_b_size == 0) {
    ...
    std::vector<unsigned int> return_wires = circuit_obj->arithmetic_functions(data_list, char_array1, NULL, input_a_size, input_b_size);
    ...
    //    const char* char_array1 = circuit_obj->read_inputs_A(key);
...
*/
       /*  std::cout << "================== Testing sha256 ========================== " << std::endl;
         test_arithmetic("../data/arithmetic-functions/sha256.txt", 512,256,256);
*/
